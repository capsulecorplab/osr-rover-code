...
class Motor(object):
    '''
    Motor class contains the methods necessary to send commands to the motor
    controllers for the corner and drive motors. There are many other ways of
    commanding the motors from the RoboClaw, we suggest trying to write your
    own Closed loop feedback method for the drive motors!
    '''
    ...
    def __init__(self, config):
        ...
        if version != 0:
            print("[Motor__init__] Sucessfully connected to RoboClaw motor \
                    controllers")
        else:
            raise Exception(
        ...
        for address in self.address:
            self.rc.SetMainVoltages(address,
                                    int(config['BATTERY_CONFIG']
                                        ['low_voltage']*10),
                                    int(config['BATTERY_CONFIG']
                                        ['high_voltage']*10)
                                    )
            if address == 131 or address == 132:
                self.rc.SetM1MaxCurrent(address, int(
                    config['MOTOR_CONFIG']['max_corner_current']*100))
                self.rc.SetM2MaxCurrent(address, int(
                    config['MOTOR_CONFIG']['max_corner_current']*100))
                ...
                self.rc.SetM1MaxCurrent(address, int(
                    config['MOTOR_CONFIG']['max_drive_current']*100))
                self.rc.SetM2MaxCurrent(address, int(
                    config['MOTOR_CONFIG']['max_drive_current']*100))
    ...
    def cornerToPosition(self, tick):
        ...
        '''
        speed, accel = 1000, 2000  # These values may need tuning
        self.errorCheck()
        for i in range(4):
            ...
            if tick[i]:
                if (i % 2):
                    self.rc.SpeedAccelDeccelPositionM2(
                        self.address[index], accel, speed, accel, tick[i], 1)
                else:
    ...
    def sendMotorDuty(self, motorID, speed):
        ...
        if speed > 0:
            if not motorID % 2:
                command = self.rc.ForwardM1
            else:
                command = self.rc.ForwardM2
        else:
            if not motorID % 2:
                command = self.rc.BackwardM1
            else:
                command = self.rc.BackwardM2
    ...
    def errorCheck(self):
        '''
        Checks error status of each motor controller, returns 0 if any errors
        occur
        '''
