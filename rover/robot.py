...
class Robot(Motor):
    ...
    def calculateVelocity(self, v, r):
        ...
        if (r == 0 or r >= 250 or r <= -250):
            ...
            # Wheels can't move faster than max (127)
            x = v/((abs(r) + self.d4))
    ...
    def getCornerDeg(self):
        ...
        for i in range(4):
            deg[i] = int(self.tick2deg(
                enc[i], self.enc_min[i], self.enc_max[i]))
        return deg

    def approxTurningRadius(self, enc):
        '''
        Takes the list of current corner angles and approximates the current
    ...
    def cornerPosControl(self, tar_enc):
        '''
        Takes the target angle and gets what encoder tick that value is for
        position control

        :param list [int] tar_enc: List of target angles in degrees for each
        corner
        '''
        tick = []
        for i in range(4):
            tick.append(self.deg2tick(
                tar_enc[i], self.enc_min[i], self.enc_max[i]))

        enc = self.getCornerEnc()
        for i in range(4):
            if abs(tick[i] - enc[i]) < 30:
                # stopping the motor when it is close to target reduced motor
                # jitter
                tick[i] = 0
    ...
    def sendCommands(self, v, r):
        '''
        Driving method for the Rover, rover will not do any commands if any
        motor controller throws an error

        :param int v: driving velocity command, % based from -100 (backward) to
         100 (forward)

        :param int r: driving turning radius command, % based from -100 (left)
         to 100 (right)
